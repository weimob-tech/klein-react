...
import warning from '../rc-util/warning';
...
export interface CascaderProps extends Pick<TriggerProps, 'getPopupContainer'> {
  ...
  onExpand?: (selectOptions: CascaderOption[]) => void;
  loadingIcon?: React.ReactNode;
  checked?: boolean;
  onPopupScroll?: (event?: any) => void; // by szp
}
...
class Cascader extends React.Component<CascaderProps, CascaderState> {
  ...
  constructor(props: CascaderProps) {
    ...
    this.state = {
      ...
      value: initialValue,
      prevProps: props,
      onPopupScroll: props.onPopupScroll, // by szp
    };
  ...
  handleMenuSelect = (
    ...
    const { changeOnSelect, loadData, expandTrigger, onExpand } = this.props;
  ...
  render() {
    ...
    if (options && options.length > 0) {
      menus = (
        <Menus
          {...this.props}
          onPopupScroll={this.state.onPopupScroll} // by szp
    ...
    return (
      ...
      >
        {React.cloneElement(children, {
          // 去除keycode监听功能，存在逻辑漏洞，并且sougou输入法输入括号会清除输入框的值
