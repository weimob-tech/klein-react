...
import useMergedState from '../rc-util/hooks/useMergedState';
import type {
  DisabledTimes,
  PanelMode,
  PickerMode,
...
function InnerRangePicker<DateType>(props: RangePickerProps<DateType>) {
  ...
  // Operation ref
  const operationRef: React.MutableRefObject<ContextOperationRefProps | null> = useRef<ContextOperationRefProps>(
    null,
  );
  ...
  function renderPanels() {
    ...
    if (!showTime) {
      ...
      const showRightPanel = picker !== 'colDate' && picker !== 'time';
      ...
      if (direction === 'rtl') {
        ...
        panels = (
          <>
            {leftPanel}
            {showDoublePanel && showRightPanel && rightPanel}
          </>
        );
      }
    } else {
  ...
  return (
    <PanelContext.Provider
      value={{
        operationRef,
        hideHeader: picker === 'time' || picker === 'colDate',
        onDateMouseEnter,
        onDateMouseLeave,
        hideRanges: true,
