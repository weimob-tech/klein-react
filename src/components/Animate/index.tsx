...
const isVisbile = (elm: HTMLDivElement, parent: HTMLDivElement) => {
    ...
    const viewBottom =
        viewTop + Math.min(parent.clientHeight, window.innerHeight);
...
const Animate: React.FC<Props> = (props) => {
    ...
    const { delay = 300, duration = 500, extraDelay = 0 } = animate || {};
    ...
    useEffect(() => {
        ...
        if (visible === undefined) {
            const parent =
                root?.() || document.querySelector(".klein-basicLayout");
    ...
    return (
        <div
            ref={ref}
            className={cx(className, { [styles.animate]: show })}
            style={{
                ...style,
