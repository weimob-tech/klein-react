...
import type { ScrollTo } from '../../rc-virtual-list/List';
...
export interface InnerSelectorProps {
  ...
  id: string;
  mode: Mode;
  prefix: React.ReactNode; // by custom --- szp
  suffix: React.ReactNode; // by custom --- szp
  itemindex: number; // by custom --- szp
...
> = (props, ref) => {
  ...
  const {
    ...
    showSearch,
    tokenWithEnter,
    prefix, // by custom --- szp
    suffix, // by custom --- szp
    itemindex, // by custom --- szp
  ...
  const onInternalInputKeyDown: React.KeyboardEventHandler<HTMLInputElement> = (
    event,
  ) => {
    const { which } = event;
  ...
  const onInputCompositionEnd: React.CompositionEventHandler<HTMLInputElement> = (
    e,
  ) => {
  ...
  const [prePadding, setPrePadding] = useState();
  const [sufPadding, setSufPadding] = useState();
  // ================= Inner Selector ==================
  const sharedProps = {
    inputRef,
    ...
    prefix, // by custom --- szp
    suffix, // by custom --- szp
    setPrePadding, // by custom --- szp
    setSufPadding, // by custom --- szp
  };
  const paddingStyle = {
    paddingLeft: prePadding,
    paddingRight: sufPadding,
  };
  ...
  return (
    <div
      ...
      style={paddingStyle} // by custom --- szp
    >
      {selectNode}
    </div>
  );
