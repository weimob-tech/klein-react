...
import useMemo from '../rc-util/hooks/useMemo';
...
import { CloseFill } from '../svg-components';
...
> = (
  ...
  // szp
  React.useEffect(() => {
    const i = itemindex || 0; // szp
    setActive(
      defaultActiveFirstOption !== false ? getEnabledActiveIndex(i) : -1, // szp
    );
    itemindex && scrollIntoView(itemindex);
  }, [memoFlattenOptions.length, searchValue, itemindex]);
  ...
  const handleRemoveOption = (e) => {
    ...
    removeOption(memoFlattenOptions[activeIndex]);
  };
    ...
    <>
      ...
      >
        {({ group, groupOption, data }, itemIndex) => {
          ...
          const optionRemoveIconClass = classNames(
            `${itemPrefixCls}-option-remove`,
            {
              [`${itemPrefixCls}-option-remove-active`]:
                activeIndex === itemIndex && !disabled && removeOption && removeable
            },
          );

          const mouseLeaveClass = () => {
            setActive(-999); // set special activeIndex for remove class   -szp
            classNames(optionClassName, {
              [`${optionPrefixCls}-active-mouseLeave`]:
                activeIndex !== itemIndex && !disabled,
            });
          };
          return (
            <div
              ...
              title={optionTitle}
              onMouseMove={() => {
                // console.log('itemIndex', itemIndex); //szp
                if (activeIndex === itemIndex || disabled) {
                  return;
                }
                setActive(itemIndex);
              }}
              onMouseLeave={mouseLeaveClass} // remove option active class onMouseLeave  -szp
              onClick={() => {
                if (!disabled) {
                  onSelectValue(value);
                }
            ...
            >
              ...
              <span
                className={optionRemoveIconClass}
