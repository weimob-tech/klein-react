...
export const getLabeledValue: GetLabeledValue<FlattenOptionData[]> = (
  ...
  const prevValItem: LabelValueType = labelInValue
    ? prevValueMap.get(value)
    : undefined;

  if (
    prevValItem &&
    typeof prevValItem === 'object' &&
    'label' in prevValItem
  ) {
  ...
  options.forEach((item) => {
    ...
    if ('options' in item) {
      ...
      if (matchGroup) {
        ...
        // Check option
        const subOptions = item.options.filter((subItem) =>
