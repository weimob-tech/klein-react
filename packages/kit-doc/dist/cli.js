...
var reactKit = require('@klein-kit/react-kit');
 ...
 */
class ReactDemoVisitor extends _default {
    // @ts-ignore
    visitImportDeclaration(n) {
        if (!n.typeOnly) {
                    ...
                    properties: n.specifiers.map((s) => {
                        ...
                        if (s.type === 'ImportDefaultSpecifier' ||
                            (s.type === 'ImportSpecifier' &&
                                ((_a = s.imported) === null || _a === void 0 ? void 0 : _a.type) === 'Identifier')) {
                            return {
                                type: 'KeyValuePatternProperty',
                                span: s.span,
                                key: s.type === 'ImportSpecifier'
                                    ? s.imported
                                    : {
                                        type: 'Identifier',
                                        span: s.span,
...
class ReactTechStack {
    ...
    transformCode(...[raw, opts]) {
        if (opts.type === 'code-block') {
            ...
            const { code } = core.transformSync(raw, {
    ...
    webpackChainConfig.plugin('ssg').use(PrerendererWebpackPlugin, [
        {
            ...
            postProcess(renderedRoute) {
                ...
                if (renderedRoute.originalRoute === '/') {
                    renderedRoute.html = renderedRoute.html.replace(`class="fp-enabled"`, '');
                }
            },
        },
    ]);
...
function init(type, option) {
    reactKit.krakenKitInit(type, {
        ...option,
        babelInclude: (includes) => includes.concat([KLEIN_PATH]),
        chainWebpack,
    });
}
function getComponentRoutes() {
    // get all component dirs
    const componentDirs = fs.readdirSync(path.resolve(process.cwd(), './packages/klein-components'));
