...
function getLeftRightPage({ footerCount, current, totalPage }) {
  const initMedian = Math.ceil((footerCount + 1) / 2);
  const preNum = initMedian - 1;
  const lastNum = footerCount - initMedian;
  let left, right;
  if (totalPage <= footerCount) {
    left = 1;
    right = totalPage;
  } else {
    if (current <= initMedian) {
      left = 1;
      right = footerCount;
    } else if (current + lastNum > totalPage) {
      right = totalPage;
      left = totalPage - footerCount + 1;
    } else {
      left = current - preNum;
      right = current + lastNum;
    }
  }
  return [left, right];
}
class Pagination extends React.Component {
  ...
  constructor(props) {
    ...
    const totalPage = calculatePage(pageSize, undefined, props);
  ...
  getItemIcon = (icon, label) => {
    ...
    let iconNode = icon || (
      ...
      >
        <span className={`${prefixCls}-icon-left`}>{label}</span>
      </button>
    );
  ...
  render() {
    ...
    if (typeof footerCount === 'number' && footerCount >= 3) {
      const pagerProps = {
        locale,
      ...
      let [pageLeft, pageRight] = getLeftRightPage({
        current,
        footerCount,
        totalPage: allPages,
      });

      for (let i = pageLeft; i <= pageRight; i += 1) {
      ...
      if (current - 1 <= pageBufferSize) {
        if (showLessItems && leftRightItemOne && current === 1) {
          right = 2;
        } else {
      ...
      if (allPages - current <= pageBufferSize) {
        if (showLessItems && leftRightItemOne && current === allPages) {
          left = allPages - 1;
        } else {
    ...
    return (
      ...
      >
        ...
        <Options
          ...
          goButton={goButton}
          getSizeChangerContainer={getSizeChangerContainer}
        />
      </ul>
    );
  }
