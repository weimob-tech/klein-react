// @ts-nocheck
import get from '../../rc-util/utils/get';
import set from '../../rc-util/utils/set';
import {
  InternalNamePath,
  NamePath,
  Store,
...
export function setValues<T>(store: T, ...restValues: T[]): T {
  return restValues.reduce((current: T, newStore: T): T => {
    return internalSetValues<T>(current, newStore);
  }, store);
}
