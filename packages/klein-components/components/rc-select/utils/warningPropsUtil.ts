...
import warning, { noteOnce } from '../../rc-util/warning';
import toNodeArray from '../../rc-util/Children/toArray';
...
function warningProps(props: SelectProps) {
  ...
  warning(
    mode !== 'tags' ||
      mergedOptions.every(
        (opt: { disabled?: boolean } & OptionGroupData) => !opt.disabled,
      ),
    ...
    const hasNumberValue = mergedOptions.some((item) => {
      if (item.options) {
        return item.options.some(
          (opt: OptionData) =>
            typeof ('value' in opt ? opt.value : opt.key) === 'number',
        );
      }
  ...
  if (children) {
    ...
    toNodeArray(children).some((node: React.ReactNode) => {
      ...
      const { type } = node as {
        type: { isSelectOption?: boolean; isSelectOptGroup?: boolean };
      };
